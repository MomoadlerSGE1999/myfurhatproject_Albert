...
//mit furhat.ask for fragt furhat nach einem spezifischem intent, in diesem Fall eine Nummer
    furhat.askFor<furhatos.nlu.common.Number>("Wie lautet ihre Patientennummer", timeout = 20000, endSil = 5000) {

        //Antwortet der Nutzer mit einer Nummer, so triggert onRepsonse<Number>
        onResponse<furhatos.nlu.common.Number> {
            //Furhat schaut den User an, der geantwortet hat
            furhat.attend(it.userId)

            //Mit it.text kann man das gesagt des Nutzers manipulieren, mit toString wird aus dem gesagten ein String gemacht
            //dann werden alle Leerzeichen mit replace ersetzt
...
//Unterscheidet sich nur in der Art und Weise wie Furhat nach der Patientennummer fragt
    furhat.askFor<furhatos.nlu.common.Number>("Was ist die Patientennummer ihres Kunden beziehungsweise Angehörigen", timeout = 20000, endSil = 5000) {
        onResponse<furhatos.nlu.common.Number> {
            furhat.attend(it.userId)
            val x: String = it.text.toString().replace(" ".toRegex(), "")
            val resultx: String = x.filter { it.isDigit() }
